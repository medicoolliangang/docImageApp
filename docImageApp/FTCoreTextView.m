...
- (NSArray *)allSubnodes
{
	NSArray *allSubnodes_ = [[self _allSubnodes] copy];
	NSMutableArray *returnedArray = [NSMutableArray arrayWithObject:self];
	[returnedArray addObjectsFromArray:allSubnodes_];
...
#ifdef __IPHONE_7_0
    suggestedSize = [_processedString boundingRectWithSize:CGSizeMake(self.bounds.size.width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:style.font} context:nil].size;
#else
...
{
    self.styles = [styles mutableCopy];
	[self didMakeChanges];
    if ([self superview]) [self setNeedsDisplay];
}
