...
- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
  ...
  for (int i = 0; i<[_indexPathsForSelectedItems count]; i++) {
    if ([asset isEqual:[_indexPathsForSelectedItems objectAtIndex:i]]) {
      temp = YES;
      break;
    }
    temp = NO;
  ...
  if (!temp) {
    if (_indexPathsForSelectedItems.count > MaxPicture) {
      UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"最多只能选择5张图片！" delegate:self cancelButtonTitle:@"知道了" otherButtonTitles:nil];
      [alert show];
      return NO;
    }
  }
