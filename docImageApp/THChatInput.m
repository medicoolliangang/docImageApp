...
- (void)layoutSubviews
{
  ...
  if ([[[UIDevice currentDevice] systemVersion] floatValue]>=8.0) {
    textView.frame = CGRectMake(x, textView.frame.origin.y-2, w, textView.frame.size.height);
  }else
...
- (void)beganEditing
{
    if (autoResizeOnKeyboardVisibilityChanged)
    {
        ...
        {
             self.transform = CGAffineTransformMakeTranslation(0, -self.keyboardHeight-44);
        } completion:^(BOOL fin){}];
        [self fitText];
    }else
    {
