...
@property (nonatomic, strong) ASIHTTPRequest *getUserInfoRequest;
@end
...
{
  [self.baseUserNameInfo resignFirstResponder];
  [self.baseUserNikeInfo resignFirstResponder];
  [self.schollInfo resignFirstResponder];
  [self.majorInfo resignFirstResponder];
  [self.studentCardInfo resignFirstResponder];
  [self.doctorLicenseInfo resignFirstResponder];
...
- (void)viewDidLoad
{
   ...
   self.getUserInfoRequest = [imdRequest getRequest:[UrlCenter urlOfType:GET_USER_INFO] delegate:self requestType:@"GET_USER_INFO"];
...
- (void)viewWillDisappear:(BOOL)animated
{
  ...
  if (self.getUserInfoRequest) {
    [self.getUserInfoRequest clearDelegatesAndCancel];
    self.getUserInfoRequest = nil;
  }
  if (request) {
...
- (void) initAllData:(NSDictionary *)userInfo
{
  if ([userInfo count] > 1) {
    NSMutableDictionary *infoDic = [NSMutableDictionary dictionaryWithDictionary:[userInfo objectForKey:@"info"]];
...
- (IBAction)saveUserInfo:(id)sender{
    ...
    {
      NSString *name = [self.modifyInfo objectForKey:USERNAME];
      NSString *realName = [self.modifyInfo objectForKey:USERREALNAME];
      if (name.length > 0) {
        NSData *nameData = [name dataUsingEncoding:NSUTF8StringEncoding];
        if (nameData.length < 4 | nameData.length > 20 | ![Strings validUserName:name]) {
          [self showAlertView:RUIYI_TITLE contentInfo:@"昵称长度必须4~20位的字符或汉字。"];
          return;
        }
      }
     if (realName.length > 0) {
      if (![Strings validRealName:realName]) {
        [self showAlertView:RUIYI_TITLE contentInfo:@"姓名必须是中文且长度大于2个字符"];
        return;
      }
     }
    }
...
- (void)sendSaveUseInfoRequest{
   request = [imdRequest postRequest:[UrlCenter urlOfType:SAVE_USERINFO] delegate:self requestType:@"SAVE_USERINFO" postData:[NSMutableDictionary dictionaryWithDictionary:[self recombinationUserModifyInfo]] json:YES];
...
{
    NSString* urlString = [NSString stringWithFormat:@"http://%@/hospital:cities?r=1352797588496",DOCIMAGEAPP_SERVER];
    ProvinceViewController *pro = [[ProvinceViewController alloc]init];
    pro.reset = @"RESET";
  [imdRequest getRequest:urlString delegate:pro requestType:@""];
...
- (void)userMobileActiveSuccess{
    [self.originInfo setObject:[NSNumber numberWithBool:YES] forKey:@"mobileVerified"];
    [self initUserInfo];
...
{
  NSDictionary *Info = [iosVersionChecker parseJSONObjectFromJSONString:request_.responseString];
  if (Info) {
    [self initAllData:Info];
  }
}

- (void)saveUserInfoFinish:(ASIHTTPRequest *)request_{
   NSDictionary *saveInfo = [iosVersionChecker parseJSONObjectFromJSONString:request_.responseString];
    ...
    if ([[saveInfo objectForKey:@"success"] boolValue]) {
        ...
        if (studentVerify || doctorVerify) {
         ...
         [userLocalData saveUserName:self.baseUserNikeInfo.text];
        //重置用户的原始信息，保证二次修改时的初始信息正确
        [self.modifyInfo removeAllObjects];
    }else{
      if ([[saveInfo objectForKey:@"usernameNotAvailable"] boolValue]) {
        [self showAlertView:RUIYI_TITLE contentInfo:@"昵称存在请重新输入"];
      }else if([[saveInfo objectForKey:@"mobileNotAvailable"] boolValue])
      {
      [self showAlertView:RUIYI_TITLE contentInfo:@"手机号已经存在"];
      }else if([[saveInfo objectForKey:@"emailNotAvailable"] boolValue])
      {
      [self showAlertView:RUIYI_TITLE contentInfo:@"邮箱已经存在"];
      }else if([[saveInfo objectForKey:@"missEmailOrMobile"] boolValue])
      {
        [self showAlertView:RUIYI_TITLE contentInfo:@"邮箱/手机号必须设置一个"];
      }else if([[saveInfo objectForKey:@"captchaNotMatch"] boolValue])
      {
      [self showAlertView:RUIYI_TITLE contentInfo:@"验证码不匹配"];
      }else if([[saveInfo objectForKey:@"invalidMobileActivationCode"] boolValue])
      {
      [self showAlertView:RUIYI_TITLE contentInfo:@"非法的激活码"];
      }else if([[saveInfo objectForKey:@"invalidData"] boolValue])
      {
        [self showAlertView:RUIYI_TITLE contentInfo:@"昵称长度需在4~20位之间"];;
      }
      else
        [self showAlertView:RUIYI_TITLE contentInfo:@"帐号信息保存失败，请稍后重试"];
    }
}
...
-(void)requestFailed:(ASIHTTPRequest*)requests
{
  ...
  if ([requests.error code] == ASIRequestTimedOutErrorType) {
    [self showAlertView:RUIYI_TITLE contentInfo:REQUEST_TIMEOUT_MESSAGE];
  }else
...
- (void)initUserInfo{
    ...
    [userLocalData saveUserName:[self.originInfo objectForKey:USERNAME]];
...
- (void)userSaveAlertViewWithBaseVerify:(BOOL)baseVerify_ studentVerify:(BOOL)studentVerify_ doctorVerify:(BOOL)doctorVerify_{
    if ([[self.originInfo objectForKey:USERTYPE] isEqualToString:USERTYPE_STUDENT]) {
        ...
        if (baseVerify_ && studentVerify_) {
            UIAlertView *alerView = [[UIAlertView alloc] initWithTitle:@"帐号信息修改确认" message:@"请您确认修改个人帐号信息。" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        ...
        if (baseVerify_ && doctorVerify_) {
            UIAlertView *alerView = [[UIAlertView alloc] initWithTitle:@"帐号信息修改确认" message:@"请您确认要修改个人帐号信息。" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
            alerView.tag = USEINFOSAVETAG;
            [alerView show];
        }else{
            [self sendSaveUseInfoRequest];
...
- (void)userRequestFinishAlertViewWithBaseVerify:(BOOL)baseVerify_ studentVerify:(BOOL)studentVerify_ doctorVerify:(BOOL)doctorVerify_{
    if ([[self.originInfo objectForKey:USERTYPE] isEqualToString:USERTYPE_STUDENT]) {
        ...
        if (doctorVerify_) {
            if (baseVerify_) {
                UIAlertView *alerView = [[UIAlertView alloc] initWithTitle:@"信息修改成功" message:@"帐号信息修改成功" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                alerView.tag = USEINFOSAVESUCCESSTAG;
                [alerView show];
            }else{
                UIAlertView *alerView = [[UIAlertView alloc] initWithTitle:@"信息修改成功" message:@"您的帐号信息修改成功" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                alerView.tag = USEINFOSAVESUCCESSTAG;
                [alerView show];
            }
        }else{
...
- (void)userCancelAlertViewWithBaseVerify:(BOOL)baseVerify_ studentVerify:(BOOL)studentVerify_ doctorVerify:(BOOL)doctorVerify_{
    ...
    if ([[self.originInfo objectForKey:USERTYPE] isEqualToString:USERTYPE_STUDENT]) {
        if (baseVerify_ && studentVerify_) {
            UIAlertView *alerView = [[UIAlertView alloc] initWithTitle:@"信息修改未保存" message:@"您的帐号信息已修改，是否保存？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
            alerView.tag = USEINFOSAVETAG;
            [alerView show];
        }else{
