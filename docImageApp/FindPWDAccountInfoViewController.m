...
- (IBAction)clickVerifyInfoBtn:(id)sender {
    ...
    if ([accountInfo rangeOfString:@"@"].location == NSNotFound) {  //手机判断
        ...
        request = [imdRequest getRequest:[NSString stringWithFormat:@"%@/%@",[UrlCenter urlOfType:CHECKMOBILE_URL],self.accountInfolabel.text] delegate:self requestType:nil];
...
#define CHECK_EMAL @"checkemail"
        ...
        request = [imdRequest getRequest:[NSString stringWithFormat:@"%@/%@",[UrlCenter urlOfType:CHECKEMAIL_URL],self.accountInfolabel.text] delegate:self requestType:nil];
   ...
   BOOL checkemail = [[request responseString] boolValue];
    if (!checkemail) {
      NSDictionary *json = @{@"device" : @"IPhone" , @"email" : self.accountInfolabel.text};
        request = [imdRequest postRequest:[UrlCenter urlOfType:FIND_EMAIL] delegate:self requestType:nil postData:[NSMutableDictionary dictionaryWithDictionary:json] json:YES];
      request.tag = 2;
          } else {
...
- (void)checkMobileInfoRequestFinished:(ASIHTTPRequest *)request_{
    ...
    if (self.type == ViewTypeFindPWD) {
        if (!checked) {
          ...
          request = [imdRequest postRequest:[UrlCenter urlOfType:FIND_MOBILE] delegate:self requestType:nil postData:[NSMutableDictionary dictionaryWithDictionary:json] json:YES];
          request.tag = 1;
        }else{
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"您输入的帐号不存在，请输入正确的手机/邮箱。" delegate:self cancelButtonTitle:@"重新输入" otherButtonTitles:@"取消", nil];
            alertView.tag = UIALERTVIEWTAG;
      ...
      if (!isOrigin) {
        ...
        if (isOrigin && self.activeType == AccountUnActived) {
          request = [imdRequest postRequest:[NSString stringWithFormat:@"%@/%@",[UrlCenter urlOfType:Mobile_VerifyCode],self.accountInfolabel.text] delegate:self requestType:@"" postData:nil json:NO];
